...
c  initialize maparrays
       call izero(maxunits*maxsequnits,mapunit,1)
       call izero(maxunits,recperunit,1)
       do ii=1,maxunits
        filename(ii)='#'
       enddo
       firstcall=.false.
...
*      mapunit(unit,1)=unit
*      filename(unit)=filnam(1:fstrlen(filnam))
*      open(unit=unit,file=filnam,
*     .   access='direct',form='unformatted',status=
*     +  'unknown',iostat=ioerr,recl=lrecl,err=900)
*@endif
c
      else
c
...
*      lrecl=2*length
*      open(unit=unit,access='direct',form='unformatted',status=
*     +  'scratch',file=filnam,iostat=ioerr,recl=lrecl,err=900)
*@elif defined  ibmcms
*c  warning *** case dependent code ***
*      TEXT='FILE '//FILNAM//' G'
*      CALL UOPEN(UNIT,TEXT,5,3,4096,4096,'FB',1,IOERR)
...
*      mapunit(unit,1)=unit
*      filename(unit)=filnam(1:fstrlen(filnam))
*      open( unit=unit, access='direct', form='unformatted',
*     & status='scratch',
*     & file=filnam, iostat=ioerr, recl=lrecl, err=900 )
*@elif defined  (rs6000) || defined (macabsoft) || defined (largefiles) 
c     # must use a filename to keep files in the correct directory.
      recperunit(unit)=1024*1024*1024
      lrecl=8*length
     ...
     & status='unknown',
     & file=filnam, iostat=ioerr, recl=lrecl, err=900 )
*@elif defined  unix sun cray convex
*c     # must use a filename to keep files in the correct directory.
*      recperunit(unit)=(maxfilesize/(8*reclen(unit)))
*      lrecl=8*length
*      mapunit(unit,1)=unit
...
c matruc: call writvc with icode instead of 3 -> was problem with ivmode=8
c matruc: calling writvc with icode instead of 3 doesn't work -> 
c         changed back to '...,loglen, 3, ...'
      call writvc(
     & unit,   lenrec, vector, start,
