ccolib5.f
ccolib part=5 of 9.  misc precision neutral (w) linpack routines.
...
c
c     blas daxpy_wr,dswap_wr,idamax_wr
...
c
      ...
      integer idamax_wr,ij,ijj,ik,ikm1,im,imax,imaxp1,imim,imj,imk
...
c
   ...
   40       continue
            if (imax .eq. 1) go to 50
               jmax = idamax_wr(imax-1,ap(im+1),1)
               jmim = jmax + im
...
c
               call dswap_wr(imax,ap(im+1),1,ap(ik+1),1)
               imj = ik + imax
               do 110 jj = imax, k
                  j = k + imax - jj
                  jk = ik + j
...
c
            ...
            do 130 jj = 1, km1
               ...
               mulk = -ap(jk)/ap(kk)
               t = mulk
               call daxpy_wr(j,t,ap(ik+1),1,ap(ij+1),1)
               ijj = ij + j
               ap(jk) = mulk
               ij = ij - (j - 1)
  130       continue
...
c              perform an interchange.
c
               call dswap_wr(imax,ap(im+1),1,ap(ikm1+1),1)
               imj = ikm1 + imax
               do 150 jj = imax, km1
                  j = km1 + imax - jj
                  jkm1 = ikm1 + j
...
c
            ...
            if (km2 .eq. 0) go to 180
               ...
               do 170 jj = 1, km2
                  ...
                  call daxpy_wr(j,t,ap(ik+1),1,ap(ij+1),1)
                  t = mulkm1
                  call daxpy_wr(j,t,ap(ikm1+1),1,ap(ij+1),1)
                  ap(jk) = mulk
                  ap(jkm1) = mulkm1
                  ijj = ij + j
                  ij = ij - (j - 1)
...
c     subroutines and functions
c
c     blas daxpy_wr,ddot_wr
c     fortran abs
c
c     internal variables.
c
      real*8 ak,akm1,bk,bkm1,ddot_wr,denom,temp
...
c
               call daxpy_wr(k-1,b(k),ap(ik+1),1,b(1),1)
   30       continue
c
c           apply d inverse.
c
...
c              apply the transformation.
c
               call daxpy_wr(k-2,b(k),ap(ik+1),1,b(1),1)
               call daxpy_wr(k-2,b(k-1),ap(ikm1+1),1,b(1),1)
   60       continue
c
c           apply d inverse.
c
...
c              apply the transformation.
c
               b(k) = b(k) + ddot_wr(k-1,ap(ik+1),1,b(1),1)
               kp = kpvt(k)
...
c
               b(k) = b(k) + ddot_wr(k-1,ap(ik+1),1,b(1),1)
               ikp1 = ik + k
               b(k+1) = b(k+1) + ddot_wr(k-1,ap(ikp1+1),1,b(1),1)
               kp = abs(kpvt(k))
